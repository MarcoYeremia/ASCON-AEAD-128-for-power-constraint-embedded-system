...
assign fsm_q = fsm_t'(fsm_q_logic);
  ...
  // FIX: escape hatch for the "AD present, message empty" case. Normally
  // ABS_MSG can only be left via abs_msg_done, which requires at least
  // one bdi_valid word to arrive -- there was no way to represent "zero
  // bytes of message" while already sitting in this state. If the host
  // (via a late bdi_eoi, e.g. triggered by issuing READ_DATA instead of
  // ever sending message bytes) tells us no message is coming and none
  // has arrived yet, treat the message as empty and move on.
  logic msg_skip;
  assign msg_skip = (fsm_q == ABS_MSG) && eoi_q && (bdi_valid == 'd0);
  ...
  always_comb begin
    ...
    // FIX: message never arrived and host has signalled no more input
    // is coming -- treat as an empty message, same target as the
    // existing "abs_msg_done with bdi_valid=='0" empty-message path.
    if (msg_skip) fsm_d = KADD_3;
...
if (dom_sep_done) begin
  ...
  if (eoi_q) begin
    state_d[0] = state_q[0] ^ 64'h0000000000000001;
  end
end

// FIX: kalau AD ada isinya, eoi_q baru ketahuan setelah FSM sudah
// lewat dari state DOM_SEP (ketahuan pas nangkring di ABS_MSG lewat
// msg_skip). Window DOM_SEP yang nge-XOR bit padding pesan kosong
// sudah kelewat, jadi terapkan XOR yang sama di sini.
if (msg_skip) begin
  state_d[0] = state_q[0] ^ 64'h0000000000000001;
end
  ...
  always_comb begin
    ...
    // FIX: while genuinely waiting for AD/message data in KADD_2 or
    // ABS_MSG, a late bdi_eoi pulse (host says "no more input coming")
    // must be able to set eoi_q so kadd_2_done/msg_skip can fire on the
    // next cycle. Previously eoi_q could only be set at ld_npub_done or
    // *_done events tied to data that had actually arrived, so an empty
    // AD/message (nothing ever arrives) had no way to signal completion.
    //
    // IMPORTANT: only treat this as "no data is coming" when bdi_valid
    // is actually 0. For short (<4 byte) messages, spi_controller can't
    // know a word is the *last* one until cs_n rises, so it delivers the
    // real data and bdi_eoi together in the same pulse. Without this
    // guard, that legitimate "final word" pulse would be misread as
    // "no message at all" if it lands while fsm_q is still KADD_2 (not
    // yet ABS_MSG), silently discarding the real data.
    if ((fsm_q == KADD_2 || fsm_q == ABS_MSG) && bdi_eoi && (bdi_valid == 'd0)) eoi_d = 'd1;
